...
static bool ReadHexBinary(LPCWSTR wszBuffer, BYTE* pBuffer, UINT uLength)
{
	...
	UINT uStrLen = (UINT)wcslen(wszBuffer);
...
{ 
	...
	if (iIndex == 0)
	{
		cType = cType.SubStr((UINT)_tcslen(OPCXML_ARRAY_PREFIX)).ToLower(0);
...
template<> bool OpcXml::WriteXml(IXMLDOMNode* ipNode, const AnyType& cValue)
{
	...
	if (iIndex == 0)
	{
		cTypeName = cTypeName.SubStr((UINT)_tcslen(OPCXML_ARRAY_PREFIX));
